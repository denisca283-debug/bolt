...
Final safety fix removes the application-source exemption: application, invitation and search active progression share current minor context/consent validation. Application candidates receive server-only immutable original project/work-hash evidence; a withdrawn application, changed context, revoked/expired consent or guardian, unreviewed opportunity or invalid responsible adult blocks every active update. Legacy candidates without context evidence remain history-only rather than receiving invented authority. Historical candidate/application RLS depends on current casting authority, separately from minor discovery; no profile/media discovery policy changes. Status-only rejection preserves all other fields and requires casting management (plus approval authority when closing an approved candidate), but never revives consent. Tests cover every active state for all three sources, rejection/reopen attacks, source/context spoofing, private historical access and guardian withdrawal. Final full-suite/CI totals are recorded on the PR heads, not inferred from code.
