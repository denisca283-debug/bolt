...
Trusted session-bound gateway → commercial_offer_discover(p_limit,p_offset), an explicit SECURITY INVOKER safe projection under initiating-user RLS. It returns only published, currently valid offers from discoverable providers, with bounded pagination (maximum 100 rows). Return factual type/claim_status/discount structure/currency/public terms and safe source classification. Evidence sourceReference must be the safe FilmVerse offer ID, never raw source_reference, provider document/quote IDs or adapter internals. Do not infer freshness from hidden provenance. Generic authenticated SELECT has only allowlisted column grants; wildcard/raw provenance queries fail. commercial_offer_provenance is a separate server-authorized provider-management RPC and is NOT part of offers.search. Trusted workers retain raw provenance access. Never turn sponsored_promotion or possible into confirmed savings. Project-specific applicability uses private commercial_offer_eligibility through its reviewed domain operation, never unrestricted rows or browser assertions. There is a canonical domain now, but no hosted tool executor or live AI integration.
