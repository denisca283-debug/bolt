...
test('real PostgreSQL races: bids, retry keys, rule edits, deadlines, wallet debit and refund',async t=>{
 ...
 const bid=(person,party,request,price,delay=false)=>as(person,`SELECT public.sourcing_bid_submit('${event}','${party}',NULL,'${request}','[{"description":"package","quantity":1,"unit_price_minor":${price}}]'::jsonb)${delay?';SELECT pg_sleep(0.2)':''}`);
 try{
 ...
 for(const name of ['20260919031153_sourcing_need_foundation.sql','20260919031615_ai_wallet_agent_foundation.sql'])await ok(await readFile(new URL('../supabase/migrations/'+name,import.meta.url),'utf8'));
 ...
 await t.test('rule revision and bid race never writes an unacknowledged new version',async()=>{
  const results=await Promise.all([run(as(b,`SELECT public.sourcing_bid_submit('${event}','${pb}',NULL,'${crypto.randomUUID()}','[{"description":"package","quantity":1,"unit_price_minor":9800}]');SELECT pg_sleep(0.2)`)),run(as(buyer,`SELECT public.sourcing_revise('${event}','Changed specification',now()+interval '1 day',20000,'Material change for concurrent test')`))]);
